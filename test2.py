...
class FlashTrackerApp(tk.Tk):
    ...
    def __init__(self):
        ...
        # A dictionary to store active timer information for the clipboard
        self.active_flash_timers = {}
    ...
    def start_timer(self, role):
        """쿨다운 타이머를 시작하고 점멸 복귀 시간을 클립보드에 복사합니다."""
        ...
        # Calculate and copy timestamp to clipboard
        try:
            ...
            future_total_seconds = current_total_seconds + cooldown
            future_minutes = future_total_seconds // 60
            future_seconds = future_total_seconds % 60

            timestamp = f"{future_minutes:02d}{future_seconds:02d}"
            clipboard_text = f"{role.lower()} {timestamp}"

            # Store the timer text and update the clipboard
            self.active_flash_timers[role] = clipboard_text
            self.update_clipboard()

        except ValueError:
            # Ignore if the game timer format is invalid
            pass

    def update_clipboard(self):
        """Sorts active timers and updates the clipboard."""
        if not self.active_flash_timers:
            self.clipboard_clear()
            return

        # Sort timers based on the timestamp (e.g., "0827")
        sorted_timers = sorted(
            self.active_flash_timers.values(), key=lambda x: x.split(" ")[1]
        )

        full_clipboard_text = " ".join(sorted_timers)

        self.clipboard_clear()
        self.clipboard_append(full_clipboard_text)
    ...
    def update_timers(self):
        """Periodically updates the cooldown labels and clipboard."""
        needs_clipboard_update = False
        for role in self.roles:
            ...
            if remaining_time > 0:
                ...
                # Check if the timer was active in the previous tick
                was_active = self.labels[role].cget("text") != "Ready"

                # Always ensure the label is correct for inactive timers
                self.labels[role].config(text="Ready", foreground="green")

                # If it just became inactive, update the clipboard
                if was_active:
                    if role in self.active_flash_timers:
                        del self.active_flash_timers[role]
                        needs_clipboard_update = True

        if needs_clipboard_update:
            self.update_clipboard()
