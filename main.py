...
class CooldownTimer:
    ...
    def activate(self):
        """Activates or restarts the timer."""
    ...
    def get_remaining_time(self):
        ...
        if elapsed >= self.spell.cooldown:
            self.is_active = False
            self.start_time = 0
            return 0
        else:
            return self.spell.cooldown - elapsed
...
class FlashTrackerApp(tk.Tk):
    ...
    def _create_widgets(self):
        ...
        for i, role in enumerate(self.roles):
            ...
            role_label.grid(row=i, column=0, padx=(0, 10), pady=5, sticky="w")

            # Cooldown status label
            self.labels[role] = ttk.Label(container, text="Ready", font=("Helvetica", 10), width=20)
            self.labels[role].grid(row=i, column=1, padx=10, pady=5, sticky="w")
            ...
            self.buttons[role].grid(row=i, column=2, padx=(0, 5), pady=5)
            ...
            reset_button.grid(row=i, column=3, padx=(0, 10), pady=5)

    def start_timer(self, role):
        """Starts or restarts the cooldown timer for a specific role."""
        self.timers[role].activate()
