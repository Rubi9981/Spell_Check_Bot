...
class FlashTrackerApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.labels = {}
        self.buttons = {}
        self.checkbox_vars = {role: {'boots': tk.BooleanVar(), 'cosmic': tk.BooleanVar()} for role in self.roles}
    ...
    def _create_widgets(self):
        ...
        ttk.Label(container, text="Ionian Boots", font=("Helvetica", 8)).grid(row=0, column=4, padx=5)
        ttk.Label(container, text="Cosmic Insight", font=("Helvetica", 8)).grid(row=0, column=5, padx=5)

        for i, role in enumerate(self.roles):
            # Role name label
            role_label = ttk.Label(container, text=f"{role}:", font=("Helvetica", 10, "bold"))
            role_label.grid(row=i + 1, column=0, padx=(0, 10), pady=5, sticky="w")

            # Cooldown status label
            self.labels[role] = ttk.Label(container, text="Ready", font=("Helvetica", 10), width=20)
            ...
            reset_button = ttk.Button(
                container,
                text="Reset",
                command=lambda r=role: self.reset_timer(r)
            )
            reset_button.grid(row=i + 1, column=3, padx=(0, 10), pady=5)

            # Checkboxes
            boots_check = ttk.Checkbutton(container, variable=self.checkbox_vars[role]['boots'])
            boots_check.grid(row=i + 1, column=4)

            cosmic_check = ttk.Checkbutton(container, variable=self.checkbox_vars[role]['cosmic'])
    ...
    def update_timers(self):
        ...
        for role in self.roles:
            ...
            if remaining_time > 0:
                self.labels[role].config(text=f"{remaining_time:.0f}s remaining", foreground="red")
            else:
